...
const FundamentusDB = require("./utils/db-fundamentus");
objFundamentusDB = new FundamentusDB();
...
(async () => {
    ...
    for (var i = 0; i < arrorg.length; i++) {
        ...
        if(papel_detalhe){
            ...
            // detalhes padrão
            if(arrorg[i].detalhes_temp['Detalhes: Papel'])                  arrorg[i].detalhes_info_nome_papel = arrorg[i].detalhes_temp['Detalhes: Papel'];
            if(arrorg[i].detalhes_temp['Detalhes: Cotação'])                arrorg[i].detalhes_info_cotacao = parseFloat(arrorg[i].detalhes_temp['Detalhes: Cotação'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Detalhes: Tipo'])                   arrorg[i].detalhes_info_tipo = arrorg[i].detalhes_temp['Detalhes: Tipo'];
            if(arrorg[i].detalhes_temp['Detalhes: Data últ cot'])           arrorg[i].detalhes_info_data_ult_cotacao = arrorg[i].detalhes_temp['Detalhes: Data últ cot'];
            if(arrorg[i].detalhes_temp['Detalhes: Últ balanço processado']) arrorg[i].detalhes_info_data_ult_balanco = arrorg[i].detalhes_temp['Detalhes: Últ balanço processado'];
            if(arrorg[i].detalhes_temp['Detalhes: Empresa'])                arrorg[i].detalhes_info_nome_empresa = arrorg[i].detalhes_temp['Detalhes: Empresa'];
            if(arrorg[i].detalhes_temp['Detalhes: Setor'])                  arrorg[i].detalhes_info_setor = arrorg[i].detalhes_temp['Detalhes: Setor'];
            if(arrorg[i].detalhes_temp['Detalhes: Subsetor'])               arrorg[i].detalhes_info_subsetor = arrorg[i].detalhes_temp['Detalhes: Subsetor'];
            if(arrorg[i].detalhes_temp['Detalhes: Min 52 sem'])             arrorg[i].detalhes_info_min_52_semanas = parseFloat(arrorg[i].detalhes_temp['Detalhes: Min 52 sem'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Detalhes: Max 52 sem'])             arrorg[i].detalhes_info_max_52_semanas = parseFloat(arrorg[i].detalhes_temp['Detalhes: Max 52 sem'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Detalhes: Vol $ méd (2m)'])         arrorg[i].detalhes_info_vol_medio_2_meses = parseFloat(arrorg[i].detalhes_temp['Detalhes: Vol $ méd (2m)'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Detalhes: Valor de mercado'])       arrorg[i].detalhes_info_valor_mercado = parseFloat(arrorg[i].detalhes_temp['Detalhes: Valor de mercado'].split('.').join('').replace(',','.'));        
            if(arrorg[i].detalhes_temp['Detalhes: Valor da firma'])         arrorg[i].detalhes_info_valor_firma = parseFloat(arrorg[i].detalhes_temp['Detalhes: Valor da firma'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Detalhes: Nro. Ações'])             arrorg[i].detalhes_info_nro_acoes = parseFloat(arrorg[i].detalhes_temp['Detalhes: Nro. Ações'].split('.').join('').replace(',','.'));
    
            // balanço patrimonial
            if(arrorg[i].detalhes_temp['Balanço Patr.: Ativo Circulante'])  arrorg[i].detalhes_balanco_ativo_circ = parseFloat(arrorg[i].detalhes_temp['Balanço Patr.: Ativo Circulante'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Balanço Patr.: Ativo'])             arrorg[i].detalhes_balanco_ativo = parseFloat(arrorg[i].detalhes_temp['Balanço Patr.: Ativo'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Balanço Patr.: Disponibilidades'])  arrorg[i].detalhes_balanco_disponibilidades = parseFloat(arrorg[i].detalhes_temp['Balanço Patr.: Disponibilidades'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Balanço Patr.: Dív. Bruta'])        arrorg[i].detalhes_balanco_div_bruta = parseFloat(arrorg[i].detalhes_temp['Balanço Patr.: Dív. Bruta'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Balanço Patr.: Dív. Líquida'])      arrorg[i].detalhes_balanco_div_liq = parseFloat(arrorg[i].detalhes_temp['Balanço Patr.: Dív. Líquida'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Balanço Patr.: Patrim. Líq'])       arrorg[i].detalhes_balanco_patrimonio_liq = parseFloat(arrorg[i].detalhes_temp['Balanço Patr.: Patrim. Líq'].split('.').join('').replace(',','.'));
    
            // demonstrativo 
            if(arrorg[i].detalhes_temp['Demonst Result. 03 MESES: EBIT'])               arrorg[i].detalhes_demons_ebit_03_meses = parseFloat(arrorg[i].detalhes_temp['Demonst Result. 03 MESES: EBIT'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Demonst Result. 03 MESES: Lucro Líquido'])      arrorg[i].detalhes_demons_ebit_03_lucro_liq = parseFloat(arrorg[i].detalhes_temp['Demonst Result. 03 MESES: Lucro Líquido'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Demonst Result. 03 MESES: Receita Líquida'])    arrorg[i].detalhes_demons_ebit_03_receita_liq = parseFloat(arrorg[i].detalhes_temp['Demonst Result. 03 MESES: Receita Líquida'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Demonst Result. 12 MESES: EBIT'])               arrorg[i].detalhes_demons_ebit_12_meses = parseFloat(arrorg[i].detalhes_temp['Demonst Result. 12 MESES: EBIT'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Demonst Result. 12 MESES: Lucro Líquido'])      arrorg[i].detalhes_demons_ebit_12_lucro_liq = parseFloat(arrorg[i].detalhes_temp['Demonst Result. 12 MESES: Lucro Líquido'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Demonst Result. 12 MESES: Receita Líquida'])    arrorg[i].detalhes_demons_ebit_12_receita_liq = parseFloat(arrorg[i].detalhes_temp['Demonst Result. 12 MESES: Receita Líquida'].split('.').join('').replace(',','.'));
    
            // oscilações
            if(arrorg[i].detalhes_temp['Oscilações: Dia'])      arrorg[i].detalhes_oscilacoes_dia = parseFloat(arrorg[i].detalhes_temp['Oscilações: Dia'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: Mês'])      arrorg[i].detalhes_oscilacoes_mes = parseFloat(arrorg[i].detalhes_temp['Oscilações: Mês'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 30 dias'])  arrorg[i].detalhes_oscilacoes_ult_30_dias = parseFloat(arrorg[i].detalhes_temp['Oscilações: 30 dias'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 12 meses']) arrorg[i].detalhes_oscilacoes_ult_12_meses = parseFloat(arrorg[i].detalhes_temp['Oscilações: 12 meses'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 2021'])     arrorg[i].detalhes_oscilacoes_ano_2021 = parseFloat(arrorg[i].detalhes_temp['Oscilações: 2021'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 2020'])     arrorg[i].detalhes_oscilacoes_ano_2020 = parseFloat(arrorg[i].detalhes_temp['Oscilações: 2020'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 2019'])     arrorg[i].detalhes_oscilacoes_ano_2019 = parseFloat(arrorg[i].detalhes_temp['Oscilações: 2018'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 2018'])     arrorg[i].detalhes_oscilacoes_ano_2018 = parseFloat(arrorg[i].detalhes_temp['Oscilações: 2017'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 2017'])     arrorg[i].detalhes_oscilacoes_ano_2017 = parseFloat(arrorg[i].detalhes_temp['Oscilações: 2017'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 2016'])     arrorg[i].detalhes_oscilacoes_ano_2016 = parseFloat(arrorg[i].detalhes_temp['Oscilações: 2016'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 2015'])     arrorg[i].detalhes_oscilacoes_ano_2015 = parseFloat(arrorg[i].detalhes_temp['Oscilações: 2015'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Oscilações: 2014'])     arrorg[i].detalhes_oscilacoes_ano_2014 = parseFloat(arrorg[i].detalhes_temp['Oscilações: 2014'].split('.').join('').replace(',','.'))/100;
    
            // indicadores fundamentalistas
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Cres. Rec (5a)'])     arrorg[i].detalhes_fund_cresc_5_anos = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Cres. Rec (5a)'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Div Br/ Patrim'])     arrorg[i].detalhes_fund_div_bruta_por_patrim_liq = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Div Br/ Patrim'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Div. Yield'])         arrorg[i].detalhes_fund_dividend_yield = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Div. Yield'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: EBIT / Ativo'])       arrorg[i].detalhes_fund_ebit_por_ativo = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: EBIT / Ativo'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: EV / EBIT'])          arrorg[i].detalhes_fund_enter_value_por_ebit = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: EV / EBIT'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: EV / EBITDA'])        arrorg[i].detalhes_fund_enter_value_por_ebitda = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: EV / EBITDA'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Giro Ativos'])        arrorg[i].detalhes_fund_giro_ativos = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Giro Ativos'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Liquidez Corr'])      arrorg[i].detalhes_fund_liq_corrente = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Liquidez Corr'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: LPA'])                arrorg[i].detalhes_fund_lucro_por_acao = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: LPA'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Marg. Bruta'])        arrorg[i].detalhes_fund_margem_bruta = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Marg. Bruta'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Marg. EBIT'])         arrorg[i].detalhes_fund_margem_ebit = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Marg. EBIT'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Marg. Líquida'])      arrorg[i].detalhes_fund_margem_liq = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: Marg. Líquida'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: ROE'])                arrorg[i].detalhes_fund_roe = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: ROE'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: ROIC'])               arrorg[i].detalhes_fund_roic = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: ROIC'].split('.').join('').replace(',','.'))/100;
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: VPA'])                arrorg[i].detalhes_fund_vpa = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: VPA'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/L'])                arrorg[i].detalhes_fund_preco_acao_por_lucro = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/L'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/VP'])               arrorg[i].detalhes_fund_preco_acao_por_valor_patrim = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/VP'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/EBIT'])             arrorg[i].detalhes_fund_preco_acao_por_ebit = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/EBIT'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: PSR'])                arrorg[i].detalhes_fund_price_sales_ratio = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: PSR'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/Ativos'])           arrorg[i].detalhes_fund_preco_acao_por_ativos_totais = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/Ativos'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/Ativ Circ Liq'])    arrorg[i].detalhes_fund_preco_acao_por_ativos_circ = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/Ativ Circ Liq'].split('.').join('').replace(',','.'));
            if(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/Cap. Giro'])        arrorg[i].detalhes_fund_preco_acao_por_capital_giro = parseFloat(arrorg[i].detalhes_temp['Indicadores fundamentalistas: P/Cap. Giro'].split('.').join('').replace(',','.'));
            
            delete arrorg[i].detalhes_temp;
            Object.keys(arrorg[i]).forEach(function(key) {
                if(isNaN(arrorg[i][key])) arrorg[i][key] = 0;
            });

            objFundamentusDB.gravaPapel(arrorg[i]);
